...
def open_email_window():
    ...
    email_window.configure(bg='light pink')

    tk.Label(email_window, text="Введите адрес электронной почты:", bg='#FAE1DF', fg='#0D1F2D').pack(pady=10)
    email_entry = tk.Entry(email_window)
    email_entry.pack(pady=10)

    send_button = tk.Button(email_window, text="Отправить", command=lambda: saada_k(email_entry.get()), bg='#0D1F2D', fg='white')
...
def open_email_window():
    ...
    email_window.title("Отправить результаты на почту")
    email_window.geometry("300x200")
    email_window.configure(bg='#FAE1DF')
    
    tk.Label(email_window, text="Введите адрес электронной почты:", bg='#FAE1DF', fg='#0D1F2D').pack(pady=10)
    email_entry = tk.Entry(email_window)
    email_entry.pack(pady=10)
    
    send_button = tk.Button(email_window, text="Отправить", command=send_email, bg='#0D1F2D', fg='white')
    send_button.pack(pady=20)
...
def calculate_and_show():
    ...
    try:
        ...
        analysis = nummodul.analyze_numbers(numbers)
...
root.configure(bg='#FAE1DF')

tk.Label(root, text="Введите дату рождения (ДД.ММ.ГГГГ):", bg='#FAE1DF', fg='#0D1F2D').pack(pady=(20, 0))
entry_date = tk.Entry(root)
entry_date.pack(pady=(0, 20))

calculate_button = tk.Button(root, text="Рассчитать", command=calculate_and_show, bg='#0D1F2D', fg='white')
calculate_button.pack(pady=(20, 20))

email_button = tk.Button(root, text="Отправить результат", command=open_email_window, bg='#0D1F2D', fg='white')
